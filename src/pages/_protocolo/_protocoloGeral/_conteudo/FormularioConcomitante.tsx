...
const FormularioConcomitante = () => {
  ...
  useEffect(() => {
    const storageKey = `dadosConcomitante_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        // Tenta buscar o campo específico 'medicacaoConcomitante'
        const conteudoApi = data.medicacao?.conteudo;
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoConcomitante: conteudoApi };
  ...
  const handleSaveApi = async (data: FormValues) => {
    ...
    // Endpoint solicitado
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/medicacao-concomitante`;

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoConcomitante,
    };
    ...
    try {
      ...
      const storageKey = `dadosConcomitante_${protocoloMestreId}`;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (protocoloMestreId) {
        navigate(`/eutanasia/${protocoloMestreId}`);
      } else {
        navigate("/eutanasia");
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <p className="text-red-500 text-sm mt-1">
                {errors.conteudoConcomitante?.message}
              </p>
            </div>
