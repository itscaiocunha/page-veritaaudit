...
const validationSchema = yup.object({
  conteudoRequisito: yup.string().required("Este campo é obrigatório."),
});
...
const FormularioRequisitos = () => {
  ...
  const {
    ...
    defaultValues: { conteudoRequisito: "" },
  });

  // useEffect para carregar dados
  useEffect(() => {
    // Chave de localStorage dinâmica
    const storageKey = `dadosRequisito_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        // Alvo da busca de dados mudou para 'requisitosRegulamentares' (suposição baseada na rota da API)
        const conteudoApi = data.requisitosRegulamentares?.conteudo; 
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoRequisito: conteudoApi };
  ...
  const handleSaveApi = async (data: FormValues) => {
    ...
    // Endpoint de POST mudou para 'requisitos-regulamentares'
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/requisitos-regulamentares`; 

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoRequisito, // Campo mudou
    };
    ...
    try {
      ...
      // Chave de localStorage dinâmica
      const storageKey = `dadosRequisito_${protocoloMestreId}`;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Navega para a próxima página, passando o ID Mestre
      navigate(`/material-metodo/${protocoloMestreId}`); 
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Label
                  htmlFor="conteudoRequisito"
                  className="text-base font-semibold text-gray-700"
                >
                  Conteúdo dos Requisitos Regulamentares
                </Label>
              </div>
              ...
              <div className="relative">
                ...
                <Textarea
                  id="conteudoRequisito"
                  {...register("conteudoRequisito")}
              ...
              <p className="text-red-500 text-sm mt-1">
                {errors.conteudoRequisito?.message}
              </p>
            </div>
