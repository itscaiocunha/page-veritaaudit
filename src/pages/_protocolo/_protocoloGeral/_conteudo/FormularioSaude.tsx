...
const validationSchema = yup.object({
  conteudoSaude: yup.string().required("Este campo é obrigatório."),
});
...
const FormularioSaude = () => {
  ...
  const {
    ...
    defaultValues: { conteudoSaude: "" },
  });

  // useEffect para carregar dados
  useEffect(() => {
    const storageKey = `dadosSaude_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        // Tenta ler de 'observacaoGeralSaude' ou 'observacaoSaude'
        const conteudoApi = data.observacaoGeralSaude?.conteudo || data.observacaoSaude?.conteudo;
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoSaude: conteudoApi };
  ...
  const handleSaveApi = async (data: FormValues) => {
    ...
    // Endpoint solicitado
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/observacao-saude`;

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoSaude,
    };
    ...
    try {
      ...
      const storageKey = `dadosSaude_${protocoloMestreId}`;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Supondo que a navegação deva incluir o ID se ele existir
      if (protocoloMestreId) {
        navigate(`/evento/${protocoloMestreId}`);
      } else {
        navigate("/evento");
      }
    } catch (error) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Label
                  htmlFor="conteudoSaude"
                  className="text-base font-semibold text-gray-700"
                >
                  Conteúdo de Observação Geral de Saúde
                </Label>
              </div>
...
export default FormularioSaude;