...
const FormularioEstatistica = () => {
  ...
  useEffect(() => {
    ...
    const storageKey = `dadosEstatistica_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        // Alvo da busca de dados mudou para 'analiseEstatistica'
        const conteudoApi = data.analiseEstatistica?.conteudo; 
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoEstatistica: conteudoApi };
  ...
  const handleSaveApi = async (data: FormValues) => {
    ...
    // Endpoint de POST mudou para 'analise-estatistica'
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/analise-estatistica`; 

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoEstatistica, // Campo mudou
    };
    ...
    try {
      ...
      // Chave de localStorage dinâmica
      const storageKey = `dadosEstatistica_${protocoloMestreId}`;
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      navigate(`/saude/${protocoloMestreId}`); 
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Label
                  htmlFor="conteudoEstatistica" // Corrigido
                  className="text-base font-semibold text-gray-700"
                >
                  Conteúdo da Análise Estatística
                </Label>
                {/* Opcional: Adicionar Tooltip <Info /> aqui */}
              </div>
              ...
              <div className="relative">
                ...
                <Textarea
                  id="conteudoEstatistica" // Corrigido
              ...
              <p className="text-red-500 text-sm mt-1">
                {errors.conteudoEstatistica?.message}
              </p>
            </div>
