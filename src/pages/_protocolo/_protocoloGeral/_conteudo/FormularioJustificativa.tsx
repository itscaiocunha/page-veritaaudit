...
const FormularioJustificativa = () => {
  ...
  useEffect(() => {
    const storageKey = `dadosJustificativa_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        const conteudoApi = data.justificativa?.conteudo;
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoJustificativa: conteudoApi };
  ...
  // --- Função: Salvar na API (Sem alterações) ---
  const handleSaveApi = async (data: FormValues) => {
    ...
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/justificativa`;

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoJustificativa,
    };
    ...
    try {
      ...
      const storageKey = `dadosJustificativa_${protocoloMestreId}`;
  ...
  // --- Função: Salvar no LocalStorage (Sem alterações) ---
  ...
  // --- Função do Botão "Salvar" (Sem alterações) ---
  ...
  // --- Função "Salvar e Avançar" (CORRIGIDA) ---
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Navega para a próxima página usando o ID MESTRE da URL
      navigate(`/requisitos/${protocoloMestreId}`);
      // --- FIM DA CORREÇÃO ---
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              ...
              <p className="text-red-500 text-sm mt-1">
                {errors.conteudoJustificativa?.message}
              </p>
            </div>
