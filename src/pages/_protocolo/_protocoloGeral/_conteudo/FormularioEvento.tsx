...
const FormularioEventoAdverso = () => {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // Salvar e Avançar
  const [isSaving, setIsSaving] = useState(false); // Salvar apenas
  ...
  const {
    ...
    defaultValues: { conteudoEventoAdverso: "" },
  });

  // --- Efeito: Carregar dados (API ou LocalStorage) ---
  useEffect(() => {
    const storageKey = `dadosEventoAdverso_${protocoloMestreId || "draft"}`;
    ...
    const fetchVersaoAtiva = async () => {
      ...
      try {
        ...
        // Mapeia o campo vindo da API (assumindo 'eventoAdverso')
        const conteudoApi = data.eventoAdverso?.conteudo;
        ...
        if (conteudoApi) {
          const dadosApi = { conteudoEventoAdverso: conteudoApi };
  ...
  const handleSaveApi = async (data: FormValues) => {
    ...
    // Endpoint solicitado
    const API_URL = `https://verita-brgchubha6ceathm.brazilsouth-01.azurewebsites.net/api/protocolo/versao/evento-adverso`;

    const body = {
      idProtocolo: protocoloVersaoId,
      conteudo: data.conteudoEventoAdverso,
    };
    ...
    try {
      ...
      const storageKey = `dadosEventoAdverso_${protocoloMestreId}`;
  ...
  // --- Função do Botão "Salvar" (Apenas Salvar) ---
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (protocoloMestreId) {
        navigate(`/medicacao-concomitante/${protocoloMestreId}`);
      } else {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Label htmlFor="conteudoEventoAdverso" className="text-base font-semibold text-gray-700">
                  Conteúdo de Evento Adverso
                </Label>
                {/* Opcional: Adicionar tooltip aqui se necessário */}
              </div>
