...
const validationSchema = yup.object({
  conteudoBibliografia: yup.string().required("Este campo é obrigatório."),
});
...
const FormularioBibliografia = () => {
  ...
  const { register, handleSubmit, formState: { errors }, reset } = useForm<FormValues>({
    ...
    defaultValues: { conteudoBibliografia: "" },
  });

  useEffect(() => {
    const savedData = JSON.parse(localStorage.getItem("dadosBibliografia") || "[]");
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const existingData = JSON.parse(localStorage.getItem("dadosBibliografia") || "[]");
      existingData.push(data);
      localStorage.setItem("dadosBibliografia", JSON.stringify(existingData));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-8 px-4 bg-gray-50 font-inter">
      ...
      <div className="w-full max-w-4xl rounded-lg p-8 bg-white shadow-md">
        <h2 className="text-2xl font-semibold text-center mb-6">13. Bibliografia</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <Textarea
              ...
              {...register("conteudoBibliografia")}
              className="min-h-[200px] mt-2"
            />
            <p className="text-red-500 text-sm mt-1">{errors.conteudoBibliografia?.message}</p>
          </div>
          ...
          <div className="flex justify-end pt-6 border-t">
            ...
            >
              {isSubmitting ? "Salvando" : "Finalizar Protocolo"}
            </Button>
          </div>
        </form>
      </div>
