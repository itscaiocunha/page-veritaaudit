...
const FormularioObjetivo = () => {
  ...
  useEffect(() => {
    const savedData = JSON.parse(localStorage.getItem("dadosObjetivo") || "[]");
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const existingData = JSON.parse(localStorage.getItem("dadosObjetivo") || "[]");
      existingData.push(data);
      localStorage.setItem("dadosObjetivo", JSON.stringify(existingData));
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-8 px-4 bg-gray-50 font-inter">
      ...
      <div className="w-full max-w-4xl rounded-lg p-8 bg-white shadow-md">
        <h2 className="text-2xl font-semibold text-center mb-6">3. Observação</h2>

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            <div className="flex items-center gap-2">
              <Label htmlFor="conteudoObjetivo" className="text-base font-semibold">
            ...
            />
            <p className="text-red-500 text-sm mt-1">{errors.conteudoObjetivo?.message}</p>
          </div>
